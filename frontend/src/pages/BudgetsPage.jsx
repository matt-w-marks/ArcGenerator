...
export default function BudgetsPage() {
  ...
  async function load() {
    const [bRes, sRes] = await Promise.all([
      api.get('/metrics/expenses/budgets'),
      api.get(`/metrics/expenses/budgets/summary?month=${thisMonth()}`),
    ]);
  ...
  async function handleSave(category) {
    setSaving(true);
    await api.put(`/metrics/expenses/budgets/${category}`, { monthly_amount: Number(editAmt) });
    setEditing(null);
    setSaving(false);
    load();
  }

  // Merge budget definitions with summary spend data
  ...
  return (
    <div className="max-w-3xl space-y-5">
      <div>
        <h2 className="text-sm font-semibold text-ink-100">Budgets</h2>
        <p className="text-xs text-ink-500 mt-0.5">
          Monthly budget allocations by category. Click an amount to edit. {format(new Date(), 'MMMM yyyy')} overview.
        </p>
      </div>
